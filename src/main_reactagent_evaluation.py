...
class Main(LoggerMixin):
    ...
    async def run(self):
        ...
        try:
            ...
            execution = ReActAgentEvaluationModeExecution(llm, evaluation_config, documents)

            await execution.run()
        except Exception as e:  # Captura cualquier otra excepción
            self.logger.info("An unexpected error occurred: %s", e)
...
if __name__ == "__main__":
    ...
    # no pasa nada si da error event loop is closed.
    asyncio.run(Main(models_name[0]).run())